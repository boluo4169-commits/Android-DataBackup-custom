...
- **修复「有个文件读不到，整个备份就标红」**：备份 `DCIM` 时如果目录里有个读不到的东西（某些机型相机建的隐藏目录 `.tmfs`，连 root 都没权限），以前整个任务会被判失败 —— 哪怕数据已完整写入、归档校验也通过了。现在这类情况不再误报。已由反馈者在他的设备上复测：同一份 DCIM 备份正常完成，不再标红，只有那一个条目不会进归档。
- **修复云端文件恢复时预处理页显示 0.00 Bytes**：列表里看得到、也勾上了，点「继续」却是 0.00 —— 因为「按来源统计」跑在了「确定来源」之前，去查了本地。现在顺序对齐了。
- **修复云端备份在「文件」列表里看不到**：切到云端后点「文件」一直是空白（也不会再出现"列表空、却写着选中 1 个项目"这种上下文错位）—— 根因是加载文件备份时把云账号参数丢掉了，永远只扫本地。现在云端那几项能正常列出来。
- **系统数据恢复时，会先告诉你哪个云账号下没有这份备份**：原来选账号的面板只看得到账号，点了才报错、而且分不清是「云端没有」还是「网络出错」；现在不存在的账号直接置灰并写明原因。
