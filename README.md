...
- **新增「系统数据」备份**：主页快捷操作新增入口，**短信**（含彩信）、**通话记录**、**WiFi 网络**三项都能备份 / 恢复。每项可选**存到本地**（用系统文件管理器挑位置）或**传到云端**（已配置的云账号，存在远端目录的 `system_data/` 下）。
- **WiFi 恢复是「重新连接」而不是覆盖配置文件**：备份出来的是一份含 SSID 与密码的网络清单，恢复时逐条把网络加回系统并连接，因此**换品牌、换机型也能用**。
- **WiFi 密码安全**：备份文件里的密码是明文，界面上有明确提示；含密码的操作一律不写进日志（日志可一键导出提交分析，避免顺带泄露）。
- **恢复前的结构体检**：恢复数据库是整库覆盖，覆盖过去本机原有的短信 / 通话库就没了。所以恢复前会先比对本机库与备份的表结构，对不上时弹窗列出缺了哪些内容、由你决定是否继续 —— 同品牌通常没有差异、不打扰你；跨品牌（一加带 `oplus_*` / `rcs_*` 字段，小米期望 `miui_*`）会如实预警。
- 设置 → 高级 →「**历史记录**」：去掉前置图标、补齐副标题，中文名称由「历史」改为「历史记录」。

### v3.11.0

- **修复云端导出误报失败**：包完整上传到云端后，本地临时文件的清理如果失败，导出会被判成失败 —— 你看到「导出失败」去重试，云端就多出一份重复的包。现在清理失败不影响导出结果。
- **WebDAV 上传补完整性校验**：以前只看服务器返回的状态码，传输被截断也算成功，而本地那份临时包随后就被删了。现在传完会核对远端文件大小。
- **上传失败清理远端半包**：FTP / SFTP / SMB / WebDAV 在传输失败、字节数对不上时，会把远端那个不完整的文件删掉。
- 导出过程分 4 段显示进度（校验 / 打包 / 校验码 / 上传），并修复导出页 ANR 与转屏导致的假失败。
...
- **修复「保护」无法撤销**：以前「保护」是个残缺的单向操作——一旦把某个备份标记为受保护（目录名加 @时间戳），就再也撤销不了，按钮会直接置灰。现在可以在「保护」和「取消保护」之间正常切换；如果已经存在一个普通版本，为避免覆盖，取消保护会被拒绝（不会丢数据）。
- **修复删除后残留空文件夹**：删除备份（本地 / 云端）后，版本目录确实删掉了，但上一层的应用文件夹会变成空壳留在存储里。现在删除时会把已空的应用目录一并清掉，扫描时也会兜底清理历史残留的空目录（有其他版本在时不会被误删）。
